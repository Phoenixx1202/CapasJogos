...
def get_resource_path(filename):
    return os.path.join(get_script_dir(), '..', 'img', filename)
...
def get_game_info(api_key,game_name, platform_id):
    try:
...
def get_game_cover(api_key,game_id):
    try:
...
def download_game_cover(api_key,game_name, platform_id, output_folder, progress_var, original_file_name, save_as_png=False, save_as_jpg=False):
    game_info = get_game_info(api_key,game_name, platform_id)
    if game_info and 'data' in game_info and 'games' in game_info['data'] and game_info['data']['games']:
        ...
        if game:
            game_id = game['id']
            game_cover = get_game_cover(api_key,game_id)
...
def start_download(folder_path, output_folder, progress_var, progress_bar, cancel_flag, save_as_png_var, save_as_jpg_var):
    ...
    for game_name, original_file_name in game_names:
        ...
        download_game_cover(api_key,game_name, platform_id, output_folder, progress_var, original_file_name, save_as_png, save_as_jpg)
        progress_bar.update()
...
# API KEY
label = tk.Label(root, text="API KEY:", background='light blue', font=('Arial', 12, 'bold'))
label.pack(pady=5)

# Campo de entrada para o usuário inserir a API KEY (largura ajustada para 64 caracteres)
api_key_entry = tk.Entry(root, font=('Arial', 12), width=64)
...
image_path = get_resource_path('img.png') # Substitua pelo caminho correto da sua imagem
